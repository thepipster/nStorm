    ...
    function cls(options){
            ...
            if (settings.useCluster){
                ...
                if (cluster.isMaster) {
                    ...
                    function forkWorker(blockName){
                        ...
                        Logger.info(blockName, "forking worker");
                    ...
                    cluster.on('exit', function(worker, code, signal) {
                        ...
                        Logger.error(blockName, 'worker ' + blockName + ' died', code, signal);
        ...
        function _startBlock(blockName){
            ...
            var block = blocks[blockName];
						...
						if (settings.debug) {
            	Logger.info(blockName, "Starting child process for " + blockName);
						}
            ...
            if (typeof block.process !== 'undefined'){
                ...
                workerPubSub.on(getPubSubHashId(blockName), function(channel, msg) {
                    if (settings.debug) {
											Logger.debug(blockName, "Got message " + msg.id);
										}
                ...
                if (block.limit == -1 || block.threads.length < block.limit){
                    ...
                    try {
                        ...
                        block.process(msg, block);
											...
											if (settings.debug) {
                        Logger.error(blockName, err.message, err.stack);
											}
                    ...
                    if (noMessages > 0 && (block.limit == -1 || block.threads.length <= block.limit)){
                        ...
                        client.lrange(getMsgQueueHashId(blockName), 0, -1, function(err, messageList){
                            ...
                            if (messageList){
                                ...
                                if (settings.debug) {
                                    Logger.debug(blockName, "Lost messages = ", lost);
                                    Logger.debug(blockName, "messageList = ", messageList);
                                    Logger.debug(blockName, "block.threads = ", block.threads);
                                }
                                            ...
                                            client.get(getMsgHashId(oldestMsgId), function(err, msgString){
                                                ...
                                                if (msgString){
                                                    ...
                                                    if (msg.replays < settings.replayLimit){
                                                        ...
                                                        if (settings.debug) {
																													Logger.error(blockName, "Message " + msg.id + " has reached replay limit, discarding");
																												}
                                                    }
                                                }
        ...
        */
        function _addSubscriber(blockName, subscriberName, filterOptions){
            ...
            if (settings.debug) {
							Logger.info(blockName, "Block '"+subscriberName+"' is subsribing to block '"+blockName+"'");
						}
        ...
        */
        ...
        function _ack(blockName, msg){
            ...
            if (settings.debug){
							Logger.info(blockName, " Message ack, took " + ms + " ms " + msg.id, block.threads);
						}
