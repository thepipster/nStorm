    ...
    var blocks = {};
        ...
        function _startBlock(blockName){
            ...
            function _doProcess(blockName, msg){
                ...
                if (block.limit == -1 || block.threads.length < block.limit){
                    ...
                    Logger.warn("[" + this.name + "] Overloaded! Can't process this message so rejecting! " + msg.id);
                }
            ...
            function _processLostMessages(blockName){
                ...
                client.llen("nstorm:msg-ids-"+blockName, function(err, noMessages){
                    ...
                    if (noMessages > 0 && (block.limit == -1 || block.threads.length <= block.limit)){
                        ...
                        client.lpop("nstorm:msg-ids-"+blockName, function(err, msgId){
                            
                            if (err) Logger.error(err);

                            if (msgId){

                                client.get("nstorm:msg-"+msgId, function(err, msgString){

                                    if (err) Logger.error(err);

                                    if (msgString){
                                        var msg = JSON.parse(msgString);
                                        Logger.warn("[" + blockName + "] Re-processing old message " + msg.id);
                                        _doProcess(blockName, msg);                         
                                    }
        ...
        */
        ...
        function _ack(blockName, msg){
            ...
            Logger.info("["+blockName+"] Message ack! " + msg.id );
