    ...
    var blocks = {};
       ...
       // var blocks = {};
        ...
        function _startBlock(blockName){
            ...
            if (typeof block.process !== 'undefined'){
                ...
                workerPubSub.on("nstorm-pubsub:"+blockName, function(channel, msg) {
                    if (debug) Logger.debug("["+blockName+"] Got message " + msg.id);
                    _doProcess(blockName, msg);
                });

                //setInterval(function(){_checkThreads(blockName)}, 200);
                //setInterval(function(){_processLostMessages(blockName)}, 300);
            }
            ...
            function _doProcess(blockName, msg, isReprocessed){
                ...
                if (debug) Logger.debug("[" + blockName + "] threads = " + block.threads.length + " limit = " + block.limit);
                ...
                if (block.limit == -1 || block.threads.length < block.limit){
                    ...
                    try {
                        ...
                        //if (isReprocessed){
                        //    Logger.warn("[" + blockName + "] Re-processing message " + msg.id, block.threads);
                        //}
                        //else {
                        //    Logger.debug("[" + blockName + "] Processing message " + msg.id, block.threads);
                        //}
                    ...
                    if (debug) Logger.warn("[" + blockName + "] Overloaded! Can't process this message so rejecting! " + msg.id);
                }
            ...
            */
            function _processLostMessages(blockName){
                ...
                client.llen("nstorm:msg-ids-"+blockName, function(err, noMessages){
                    ...
                    if (noMessages > 0 && (block.limit == -1 || block.threads.length <= block.limit)){
                        ...
                        client.lrange("nstorm:msg-ids-"+blockName, 0, -1, function(err, messageList){
                            ...
                            if (messageList){
                                ...
                                if (debug) Logger.error("Lost messages = ", lost);
                                if (debug) Logger.error("messageList = ", messageList);
                                if (debug) Logger.error("block.threads = ", block.threads);
                                ...
                                if (lost && lost.length > 0){
                                    ...
                                    client.lrem("nstorm:msg-ids-"+blockName, -1, oldestMsgId, function(err){
                                        ...
                                        if (err) {
                                            ...
                                            client.get("nstorm:msg-"+oldestMsgId, function(err, msgString){
        ...
        */
        function _addSubscriber(blockName, subscriberName, filterOptions){
            ...
            if (debug) Logger.info("Block '"+subscriberName+"' is subsribing to block '"+blockName+"'");
        ...
        function _emit(blockName, msg){
            ...
            for (var i=0; i<block.subscribers.length; i++){
                ...
                if (_isFiltered(msg, block.subscribers[i].filter)){
                    ...
                    client.set("nstorm:msg-"+msg.id, JSON.stringify(msg))
                    client.lpush("nstorm:msg-ids-"+msg.target, msg.id);

                    pubsub.emit("nstorm-pubsub:"+msg.target, msg);
        ...
        */
        ...
        function _ack(blockName, msg){
            ...
            if (debug) Logger.info("["+blockName+"] Message ack! " + msg.id, block.threads);

            _clearThread(block, msg.id);

            // Remove this message from my list of messages as its been processed
            client.lrem("nstorm:msg-ids-"+blockName, -1, msg.id);
            client.del("nstorm:msg-"+msg.id);
