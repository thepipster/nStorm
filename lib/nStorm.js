...
nStorm = (function() {
    ...
    function cls(options){
        ...
        if (settings.debug) {
            Logger.info("nStorm settings ", settings);
        }
            ...
            if (settings.useCluster){
                ...
                if (cluster.isMaster) {
                    ...
                    function forkWorker(blockName){
                        ...
                        Logger.infoX(blockName, "forking worker");
                    ...
                    cluster.on('exit', function(worker, code, signal) {
                        ...
                        Logger.errorX(blockName, 'worker ' + blockName + ' died', code, signal);
        ...
        function _startBlock(blockName){
            ...
            Logger.infoX(blockName, "Starting child process for " + blockName);
            ...
            if (typeof block.process !== 'undefined'){
                ...
                workerPubSub.on(getPubSubHashId(blockName), function(channel, msg) {
                    if (settings.debug) Logger.debugX(blockName, "Got message " + msg.id);
            ...
            function _doProcess(blockName, msg, isReprocessed){
                ...
                if (settings.debug) Logger.debugX(blockName, " threads = " + block.threads.length + " limit = " + block.limit);
                ...
                if (block.limit == -1 || block.threads.length < block.limit){
                    ...
                    try {
                        ...
                        if (settings.debug) {
                            if (isReprocessed){
                                Logger.warnX(blockName, " Re-processing message " + msg.id + ". Replay count = " + msg.replays, block.threads);
                            }
                            else {
                                Logger.debugX(blockName, " Processing message " + msg.id + ". Replay count = " + msg.replays, block.threads);
                            }                            
                        }
                        ...
                        Logger.errorX(blockName, err.message, err.stack);
                        _clearThread(block, msg.id);
                    }

                }
                else {
                    if (settings.debug) Logger.warnX(blockName, " Overloaded! Can't process this message so rejecting! " + msg.id);
                }
            ...
            */
            function _processLostMessages(blockName){
                ...
                client.llen(getMsgQueueHashId(blockName), function(err, noMessages){
                    ...
                    //    Logger.warnX(blockName, " " + noMessages + " unprocessed messages in the queue");                                                
                    //}
                    ...
                    if (noMessages > 0 && (block.limit == -1 || block.threads.length <= block.limit)){
                        ...
                        client.lrange(getMsgQueueHashId(blockName), 0, -1, function(err, messageList){
                            ...
                            if (messageList){
                                ...
                                if (settings.debug) {
                                    Logger.debugX(blockName, "Lost messages = ", lost);
                                    Logger.debugX(blockName, "messageList = ", messageList);
                                    Logger.debugX(blockName, "block.threads = ", block.threads);
                                }
                                ...
                                if (lost && lost.length > 0){
                                    ...
                                    client.lrem(getMsgQueueHashId(blockName), -1, oldestMsgId, function(err){
                                        ...
                                        if (err) {
                                            Logger.errorX(blockName, err);
                                        }
                                        else {

                                            client.get(getMsgHashId(oldestMsgId), function(err, msgString){

                                                if (err) LoggerX.error(blockName, err);
                                                ...
                                                if (msgString){
                                                    ...
                                                    if (msg.replays < settings.replayLimit){
                                                        ...
                                                        if (settings.debug) Logger.errorX(blockName, "Message " + msg.id + " has reached replay limit, discarding");   
                                                    }
                                                }
        ...
        */
        function _addSubscriber(blockName, subscriberName, filterOptions){
            ...
            if (settings.debug) Logger.infoX(blockName, "Block '"+subscriberName+"' is subsribing to block '"+blockName+"'");
        ...
        */
        ...
        function _ack(blockName, msg){
            ...
            if (settings.debug) Logger.infoX(blockName, " Message ack, took " + ms + " ms " + msg.id, block.threads);
