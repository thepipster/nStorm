    ...
    function cls(options){
        ...
        function _startBlock(blockName){
            ...
            if (typeof block.process !== 'undefined'){
                ...
                workerPubSub.on(getPubSubHashId(blockName), function(channel, msg) {
                    if (settings.debug) {
											Logger.debugX(blockName, "Got message " + msg.id);
										}
            ...
            function _doProcess(blockName, msg, isReprocessed){
                ...
                if (!block){
                    Logger.error("Block " + blockName + " not defined!?");
                    return;
                }


                if (settings.debug) {
									Logger.debugX(blockName, " threads = " + block.threads.length + " limit = " + block.limit);
								}
                ...
                if (block.limit == -1 || block.threads.length < block.limit){
                    ...
                    if (settings.debug) {
											Logger.warnX(blockName, " Overloaded! Can't process this message so rejecting! " + msg.id);
										}
                }
                                            ...
                                            client.get(getMsgHashId(oldestMsgId), function(err, msgString){
                                                ...
                                                if (msgString){
                                                    ...
                                                    if (msg.replays < settings.replayLimit){
                                                        ...
                                                        if (settings.debug) {
																													Logger.errorX(blockName, "Message " + msg.id + " has reached replay limit, discarding");
																												}
                                                    }
                                                }
        ...
        */
        function _addSubscriber(blockName, subscriberName, filterOptions){
            ...
            if (settings.debug) {
							Logger.infoX(blockName, "Block '"+subscriberName+"' is subsribing to block '"+blockName+"'");
						}
        ...
        */
        ...
        function _ack(blockName, msg){
            ...
            if (settings.debug){
							Logger.infoX(blockName, " Message ack, took " + ms + " ms " + msg.id, block.threads);
						}
